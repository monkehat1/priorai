...
self.addEventListener('fetch', e => {
  ...
  if (url.hostname==='api.anthropic.com' || url.pathname.includes('netlify/functions')) {
  ...
  e.respondWith(caches.match(e.request).then(cached=>cached||fetch(e.request).then(r=>{if(r&&r.status===200){const c=r.clone();caches.open(CACHE).then(cache=>cache.put(e.request,c));}return r;})));
});
